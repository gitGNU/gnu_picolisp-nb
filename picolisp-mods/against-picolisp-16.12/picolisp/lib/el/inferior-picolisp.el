...
;;; Guillermo R. Palavecino, Thorsten Jolitz, Christopher Howard
...
;; Christopher:
;; christopher dot howard at qlfiles dot net

;;;; Changes from Christopher:
;; Added function picolisp-load-current-file which makes it a bit more
;; convenient to reload the file you are working on
...
(defconst inferior-picolisp-version "1.2-patch-1"
...
(defvar inferior-picolisp-mode-map
  (let ((m (make-sparse-keymap)))
    ...
    (define-key m "\C-c\C-a" 'picolisp-load-current-file)
    m ) )
...
(define-key picolisp-mode-map "\C-c\C-a" 'picolisp-load-current-file)
...
(let ((map (lookup-key picolisp-mode-map [menu-bar picolisp])))
  ...
  (define-key map [load-file]
    '("(Re)Load Current Picolisp File" . picolisp-load-current-file) )
...
(defun picolisp-load-current-file ()
  "Load the file matching the current buffer."
  (interactive)
